...
if (is_admin()) :
	...
	?>
	<div class="wrap">
		<h2><?php echo __( 'Options for Scriptura framework', 'scriptura' ); ?></h2>
		<form method="post" action="">
			...
			<h3>Google Analytics</h3>
				<tbody>
					<tr>
						<th scope="row">
							<label><?php echo __( 'Account number', 'scriptura' ); ?></label>
						</th>
						<fieldset>
						<td>
							...
							<p class="description"><?php echo __( 'Register your account number Analytics if you require this service. You must have previously created an account on <a href="http://www.google.fr/intl/fr/analytics/">the API website</a>.', 'scriptura' ); ?></p>
						</td>
						<fieldset>
					</tr>
				</tbody>
			...
			<h3>Google Maps</h3>
				<tbody>
					<tr>
						<th scope="row">
							<label><?php echo __( 'API Key', 'scriptura' ); ?></label>
						</th>
						<fieldset>
						<td>
			...
			<h3>Google reCAPTCHA</h3>
				<tbody>
					<tr>
						<th scope="row">
							<label><?php echo __( 'Recaptcha Site Key', 'scriptura' ); ?></label>
						</th>
						<fieldset>
						<td>
					...
					<tr>
						<th scope="row">
							<label><?php echo __( 'Recaptcha Secret Key', 'scriptura' ); ?></label>
						</th>
						<fieldset>
						<td>
			...
			<hr>
			<table class="form-table">
			<h3><?php echo __( 'Default thumbnail', 'scriptura' ); ?></h3>
			<p><?php echo __( 'Customizing the default image for the presentation of items.', 'scriptura' ); ?></p>
				<tbody>
					<tr>
						<th scope="row">
							<label for="scriptura_def_thumbnail"><?php echo __( 'Default thumbnail', 'scriptura' ); ?></label>
						</th>
						<td>
			...
			<hr>
			<table class="form-table">
			<h3><?php echo __( 'Snowstorm', 'scriptura' ); ?></h3>
				<tbody>
					<tr>
						<th scope="row">
							<label for="scriptura_snowstorm"><?php echo __( 'Activate', 'scriptura' ); ?></label>
						</th>
						<td>
						<select id="scriptura_calendar" name="option[scriptura_snowstorm]">
							<option value="0"<?php if(get_option('scriptura_snowstorm') == 0) { echo ' selected="selected"'; } ?><?php if (!get_option('scriptura_snowstorm')) { echo ' selected="selected"'; } ?>><?php echo __( 'No', 'scriptura' ); ?></option>
							<option value="1"<?php if(get_option('scriptura_snowstorm') == 1) { echo ' selected="selected"'; } ?>><?php echo __('Yes', 'scriptura'); ?></option>
						</select>
						<p class="description"><?php echo __( 'Displays a snow effect on the site during the winter season.', 'scriptura' ); ?></p>
						</td>
					</tr>
				</tbody>
			</table>
