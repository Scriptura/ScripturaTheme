...
// -----------------------------------------------------------------------------
// @section     Function Moon Phases
// @description Calcul des lunaisons
// @author      Samir Shah
// @link        http://rayofsolaris.net
// @licence     MIT
// -----------------------------------------------------------------------------

// @link https://github.com/solarissmoke/php-moon-phase
// @note Le namespace n'est pas obligatoire... mais c'est plus fun ! PHP 5.3 requis
// Adapted for PHP from Moontool for Windows
// @link http://www.fourmilab.ch/moontoolw/
// @params :
//         phase() : fraction angulaire d'un cercle complet (0 et 1 = Nouvelle Lune, 0,5 = Pleine Lune)
//  illumination() : fraction éclairée de la Lune (0 = Nouvelle Lune, 1 = Pleine Lune)
//           age() : âge de la Lune en jours
//      distance() : distance de la Lune par rapport au centre de la terre (en km)
//      diameter() : diamètre angulaire de la Lune pour un observateur au centre de la Terre (en degrés)
//   sundistance() : distance par rapport au Soleil (en kilometres)
//   sundiameter() : diamètre angulaire du Soleil pour un observateur au centre de la Terre (degrés)
//      new_moon() : moment de la dernière Nouvelle Lune (UNIX timestamp)
// next_new_moon() : moment de la prochaine Nouvelle Lune (UNIX timestamp)
//     full_moon() : moment de la pleine Lune pour le cycle lunaire actuel (UNIX timestamp)
// first_quarter() : moment de la pleine Lune pour le cycle lunaire actuel (UNIX timestamp)
//  last_quarter() : temps du dernier quart dans le cycle lunaire actuel (UNIX timestamp)
//    phase_name() : nom de la phase lunaire en cours
...
// Fonction perso pour Scriptura
function ScripturaMoonCalendar() {
	$moon = new MoonPhase();
	$age = round($moon->age(), 1);
	$stage = $moon->phase() < 0.5 ? 'ascendante' : 'descendante';
	$phase = $moon->phase_name();
	$distance = round( $moon->distance(), 2 );
	$next = gmdate('j.m.Y', $moon->next_new_moon());
		$obj .= '<li>Phase lunaire&nbsp;: ' . $phase . '</li>';
		$obj .= '<li>Âge de la lune pour le cycle actuel&nbsp;: ' . $age . ' jours</li>';
		$obj .= '<li>Phase ' . $stage . '</li>';
		$obj .= '<li>Distance de la Terre&nbsp;: ' . $distance . ' km</li>';
		$obj .= '<li>Prochaine lune le ' . $next . '</li>';
	return $obj;
}

function ScripturaMoonPhases( $size ) {
	// @param $size : taille de la lune en rem
	$moon = new MoonPhase();
	$phase_moon = round($moon->phase() * 16);
		$obj .= '<div class="center" style="background-image:url(' . get_template_directory_uri() . '/Images/Moons/moon' . $phase_moon . '.svg);background-size:cover';
		if ( $size )
			$obj .= ';max-width:' . $size . 'rem';
		$obj .= '">';
		$obj .= '<div style="padding-bottom:100%"></div>';
		$obj .= '</div>';
	return $obj;
}
